...
export default function CourseBlock ({ course }) {
  ...
  const SaveStudentRecent = async id => {
    ...
    axios
      .post('/api/user/updateRecentUserData', data)
      .then(res => {})
      .catch(Error => {
        console.log('Error : ', Error)
      })

    history.push('mycoursePlayer/' + id)
  }
  return (
    <>
        ...
        <div className='row no-gutters'>
          ...
          <div className='col-md-6'>
            <div className='card-body'>
              ...
              <p className='card-text'>{console.log("date formate" , typeof(course.modifiedDate) )}
                <small className='text-muted'>Last updated 3 mins ago</small>
              </p>
              <Button
                variant='primary'
                onClick={() => {
                  history.push('/MyCourses/Course/' + course._id)
                }}
              >
                Course detail
              </Button>
              ...
              <Button
                variant='success'
                onClick={() => {
                  history.push('MyCourses/' + course._id + '/Tests')
                }}
              >
                Test
              </Button>
